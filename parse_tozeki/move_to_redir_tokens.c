...
/*   Updated: 2024/04/25 21:41:22 by tozeki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_token	*_separate_and_append_redir_tokens(t_token **redir_tkn_head, t_token **cmd_tkn_head, t_token *first)
{
	t_token *last;
	t_token *next_ptr;
	t_token *prev;
...
static t_token *_make_redir_tokens_list(t_token **cmd_tkn_head, t_bool (*is_redir_func)(enum e_token_kind))
{
	...
	while (ptr != NULL)
	{
		if (is_redir_func(ptr->kind))
			ptr = _separate_and_append_redir_tokens(&redir_tkn_head, cmd_tkn_head, ptr);
		else
			ptr = ptr->next;
	}
